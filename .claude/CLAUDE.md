...
- An endpoint in water is planned as a combined route, not a sea one: it boards nothing and sails from where it is, so water→land pairs are `footShip`/`horseShip`. `ship` stays port↔port and water↔water
- Three planners fill one `RoutePlan`: `planGroundRoute` (`foot`/`horse`), `planSeaRoute` (`ship`) and `planCombinedRoutes` (`footShip`/`horseShip`, walk to a port → sail → walk on). Each `RouteResult` carries its own `legs`, and every leg its own `distanceKm`/`timeHours`. A ship is searched on one fixed grid covering the whole map, built once per index and cached (`getSeaRaster`); a second, coarser one ranks candidate ports
- A combined route is not planned at all when a straight-line bound (`getBestCombinedBound`) shows it cannot beat the ground route - two inland towns always have one on paper, and computing it was the most expensive thing a request did
- Which port a combined route uses is the one judgement in the planner that is not "fastest wins": among ports whose walk is within `PORT_APPROACH_BAND` of the nearest, the best `PORT_TYPE_PRIORITY` type wins, unless that more than doubles the journey (`PORT_PRIORITY_TIME_CAP`). Both constants are meant to be tuned; the reasoning and the measurements behind them are in `.claude/docs/routing.md`
