...
|--------|----------|------|
| `src/app/utils/raster.ts` | [raster.md](raster.md) | Turns geodata into a queryable index and into a numeric grid of terrain coefficients |
| `src/app/utils/routing.ts` | [routing.md](routing.md) | The planner: grid search, road/grid variant selection, post-processing, cost accounting |
...
bounds, landmasses — are stated as requirements in a separate specification, kept outside the
repository. Where a rule is quoted here, the quote is the authority available to you; the numbering
...
`horse = 8`, `dragon = 100`. Foot and horse share one route and one cost and differ only by the
divisor.

**Leg.** One contiguous stretch of a route with a single mode: `kind: 'road' | 'grid'`. The legs of a
finished route **partition** it — concatenating their paths in order reproduces the drawn path point
for point (`checkLegPartition`), so anything that styles or diagnoses a leg is working with the same
line the user sees.

## How one request flows

```
RoutingGeodata ──buildRoutingIndex──► RoutingIndex            (once, at worker init, ~40 ms)
                                          │
from, to ─────────────────────────────────┼──► planRoutesWithIndex
                                          ...
                                          ▼
                                     RoutePlan { foot, horse, dragon, legs }
```

`buildGridRoute` is the inner workhorse and always runs the same four steps: rasterise a grid →
A* over it → simplify and pull the path taut → re-measure its cost along the drawn line.
...
`isOnOneLandmass`), before any grid exists.
...
Current numbers, from the harness over 17 route cases against real geodata:

- total ~700 ms for 17 cases, worst case ~140 ms, impossible pairs 0 ms, pure-road routes 0–1 ms
- `buildRoutingIndex` ~40 ms, of which nearly all is `indexLandmasses`; it runs once per worker
- before the refactor these cases took 15 301 ms total with a 6 486 ms worst case
