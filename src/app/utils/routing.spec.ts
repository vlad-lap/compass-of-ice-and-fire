...
describe('classifyCell', () => {
    ...
    it('treats a sea as impassable water', () => {
        expect(classifyCell([4, 7], index, CELL_SIZE)).toBeNull();
    });
...
describe('map bounds', () => {
    ...
    it('never routes around the edge of the map', () => {
        ...
        const wall: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([rect([MapBounds.West + 1.9, MapBounds.North - 40], 0.2, 41)]),
        };
...
describe('buildRoutingIndex', () => {
    ...
    it('keeps only non-dry lakes in the lakes layer, and seas separate', () => {
        const index = buildRoutingIndex(geodata);

        expect(index.lakes).toHaveLength(1);
        expect(index.seas).toHaveLength(1);
        expect([...index.lakes, ...index.seas].every(area => area.k === null)).toBe(true);
    });
});
...
describe('findPath', () => {
    ...
    it('returns null when a solid wall of water fully separates start and goal', () => {
        const wallGeodata: RoutingGeodata = { ...emptyRoutingGeodata(), seas: collection([rect([5, 0], 1, 10)]) };
    ...
    it('routes through a gap in a wall of water', () => {
        const wallWithGapGeodata: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([rect([5, 0], 1, 5), rect([5, 6], 1, 4)]),
        };
    ...
    it('returns null immediately when the start or goal cell is impassable', () => {
        const index = buildRoutingIndex({ ...emptyRoutingGeodata(), seas: collection([rect([0, 0], 10, 10)]) });
...
describe('placePoint', () => {
    ...
    const placementGeodata: RoutingGeodata = {
        ...
        lakes: collection([
            rect([2, 2], 3, 3),
            rect([0, 7], 2, 2, { variant: 'dry' }),
        ]),
        seas: collection([rect([6, 6], 3, 3)]),
    };
    ...
    it('rejects a point in open sea instead of snapping it ashore', () => {
    ...
    it('does not snap a point in a dry lake, since it is already passable', () => {
        expect(placePoint([0.5, 7.5], placementGrid, index, placementRaster)).toEqual({ col: 0, row: 7 });
    });
...
describe('planRoutes', () => {
    ...
    it('returns null foot/horse when a wall of water fully separates the points, but still returns dragon', () => {
        ...
        const wallGeodata: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([rect([5, -100], 1, 200)]),
        };
    ...
    it('returns null foot/horse when either point lands in open sea', () => {
        const from: [number, number] = [0, 0];
        const to: [number, number] = [10, 10];
        const geodataWithSea: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([rect([9, 9], 2, 2)]),
        };
...
describe('planRoutes with roadNetwork', () => {
    ...
    it('skips a nearer-but-unreachable exit node in favor of a farther one that actually works', () => {
        ...
        // Node [6, 2] is numerically closer to the target than [6, -5], so it's tried first - but
        // it sits in open sea itself (unreachable no matter the detour), while [6, -5] approaches
        // the target from clear, water-free ground.
        const geodataWithWall: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([square([5, 1], 2)]),
        };
    ...
    it('falls back to a direct route when the nearest-to-target network node is a dead end', () => {
        ...
        // Node [5, 5] is numerically closer to the target than [0, 0], so it gets picked as the
        // exit point - but it sits in open sea, so the grid leg from there can never succeed.
        const geodataWithDeadEndNode: RoutingGeodata = {
            ...emptyRoutingGeodata(),
            seas: collection([rect([4, 4], 2, 2)]),
        };
