...
export function placePoint(
    ...
    if (!isPointInAreas(point, index.land) || isPointInAreas(point, index.seas)) {
        return null;
    }
...
// The four cases of .claude/routing.md. Every road-assisted route has one of two shapes:
...
// Ties the route to the actual endpoints and drops the duplicates the leg joints leave behind: a
// grid leg starts at the center of the cell containing its start, not at the point itself, and a
// road leg starts at its first network node, so without this the drawn line has visible stubs and
// the measured length omits them.
function anchorRoute(route: GroundRoute | null, from: Position, to: Position): GroundRoute | null {
    ...
    return {
        rawPath: anchorPath(route.rawPath, from, to),
        renderPath: anchorPath(route.renderPath, from, to),
        cost: route.cost,
        legs: route.legs,
    };
}
...
function anchorPath(path: Position[], from: Position, to: Position): Position[] {
    const anchored: Position[] = [from];

    for (const position of [...path, to]) {
        const last = anchored[anchored.length - 1];
        if (Math.abs(position[0] - last[0]) > JOINT_EPSILON || Math.abs(position[1] - last[1]) > JOINT_EPSILON) {
...
function combineRoutes(...pieces: (GroundRoute | null | false)[]): GroundRoute {
    const combined: GroundRoute = { rawPath: [], renderPath: [], cost: 0, legs: [] };

    for (const piece of pieces) {
        if (!piece) {
            continue;
        }
        combined.rawPath.push(...piece.rawPath);
        combined.renderPath.push(...piece.renderPath);
...
function roadSegmentRoute(roadPath: RoadNetworkPath): GroundRoute {
    return {
        rawPath: roadPath.path,
        renderPath: roadPath.path,
...
function buildGridRoute(gridFrom: Position, gridTo: Position, index: RoutingIndex): GroundRoute | null {
    const { groundPath, grid } = findGroundPathWithRetry(gridFrom, gridTo, index);
    if (!groundPath) {
        return null;
    }
    ...
    return {
        rawPath: groundPath.path,
        renderPath: smoothedGridPath,
        cost: groundPath.cost,
        legs: [{ kind: 'grid', path: smoothedGridPath, cost: groundPath.cost }],
    };
}
...
function findGroundPathWithRetry(
    ...
    to: Position,
    index: RoutingIndex,
): { groundPath: PathResult | null; grid: Grid } {
    let grid: Grid;
    ...
    for (const { marginRatio, minMargin, cellBudget } of MARGIN_RETRY_STEPS) {
        ...
        if (groundPath) {
            return { groundPath, grid };
        }
    }

    return { groundPath: null, grid };
}
...
function toRouteResult(route: GroundRoute, speedKmh: number): RouteResult {
    return {
        path: route.renderPath,
        distanceKm: getPathDistanceKm(route.renderPath),
