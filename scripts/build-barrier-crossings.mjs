...
export function buildBarrierCrossings(roadsCollection, riversCollection, wallCollection, locationsCollection) {
    ...
    for (const location of locationsCollection.features) {
        ...
        // A crossing per river, not one for the nearest: a town on a confluence guards both rivers,
        // and crossings are matched per barrier, so declaring only the nearest left the other river
        // blocking the town itself. Riverrun, Godsgrace, Ny Sar and The Sorrows all sat on one.
        for (const river of rivers) {
            let nearest = null;
            ...
            for (const riverPart of getLineParts(river)) {
                for (let j = 0; j < riverPart.length - 1; j++) {
                    ...
                    if (distance <= threshold && (!nearest || distance < nearest.distance)) {
                        nearest = { distance, point: onRiver };
                    }
                }
            }

            if (nearest) {
                // Snapped onto the river rather than kept at the location, so the hole the routing
                // opens is centred on the water it has to get across and can stay small.
                crossings.push({
                    point: nearest.point,
                    kind: 'location',
                    barrier: getFeatureLabel(river),
